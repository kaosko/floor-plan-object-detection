#!/usr/bin/env python3
"""
Analytical Symbol Detection - Streamlit UI
Modern interactive interface for floor plan symbol detection
"""
...
class SymbolDetectionApp:
    ...
    def render_sidebar(self):
        ...
        if st.session_state.pdf_loaded:
            ...
            try:
                ...
                st.error("Invalid scale format. Please use comma-separated numbers.")
            ...
            try:
                ...
                st.error("Invalid angle format. Please use comma-separated numbers.")
    ...
    def render_image_processing_tab(self):
        ...
        with col2:
            if st.session_state.current_image is not None:
                st.markdown('<div class="success-box">✅ Page rendered successfully</div>', 
                          unsafe_allow_html=True)
    ...
    def render_roi_selection_tab(self):
        ...
        if st.session_state.current_image is None:
            st.markdown('<div class="warning-box">⚠️ Please render a PDF page first</div>', 
                       unsafe_allow_html=True)
            return
        ...
        with col1:
            if st.button("✂️ Select ROI Interactively", use_container_width=True):
                ...
                self.interactive_roi_selection()
    ...
    def render_detection_tab(self):
        ...
        if not st.session_state.template_extracted:
            st.markdown('<div class="warning-box">⚠️ Please select ROI and extract template first</div>', 
                       unsafe_allow_html=True)
            return
        ...
        with col2:
            if st.session_state.detection_results:
                ...
                st.markdown(f'<div class="success-box">✅ Found {num_detections} symbols</div>', 
                           unsafe_allow_html=True)
    ...
    def render_pdf_page(self):
        ...
        try:
            ...
            st.error(f"Failed to render PDF page: {str(e)}")
    
    def interactive_roi_selection(self):
        """Interactive ROI selection using OpenCV"""
        try:
            roi_selector = ROISelector()
            roi = roi_selector.select_interactive(st.session_state.current_image)
            if roi:
    ...
    def extract_template(self):
        ...
        if st.session_state.config.roi and st.session_state.current_image is not None:
            try:
                ...
                st.error(f"Template extraction failed: {str(e)}")
    ...
    def run_detection(self):
        ...
        try:
            ...
            st.success(f"Detection completed! Found {len(results.detections)} symbols")
        except Exception as e:
    ...
    def render_detection_preview(self):
        ...
        if st.session_state.detection_results and st.session_state.current_image is not None:
            try:
                ...
                st.error(f"Failed to render detection preview: {str(e)}")
    ...
    def render_results_table(self):
        ...
        try:
            ...
            st.error(f"Failed to create results table: {str(e)}")
    ...
    def load_roi(self):
        ...
        try:
            roi_path = st.file_uploader("Select ROI file", type=['json'])
            if roi_path:
                roi_data = json.load(roi_path)
                st.session_state.config.roi = tuple(roi_data)
                st.session_state.roi_selected = True
                self.extract_template()
                st.success("ROI loaded successfully!")
        except Exception as e:
            st.error(f"Failed to load ROI: {str(e)}")
    ...
    def export_yolo_labels(self):
        ...
        if st.session_state.detection_results:
            try:
                ...
                st.info("YOLO export functionality coming soon!")
            except Exception as e:
                st.error(f"Failed to export YOLO labels: {str(e)}")
    ...
    def save_annotated_image(self):
        ...
        if st.session_state.detection_results and st.session_state.current_image is not None:
            try:
                # Implementation would go here
                st.info("Annotated image save functionality coming soon!")
            except Exception as e:
                st.error(f"Failed to save annotated image: {str(e)}")
